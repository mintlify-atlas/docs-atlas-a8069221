---
title: "OpenAI Facade — PHP AI SDK API Reference"
sidebarTitle: "OpenAI Facade"
description: "Complete API reference for the OpenAI facade and OpenAIProvider class — static methods, constructor options, and return types extracted from source."
---
...
<Note>
  Custom headers passed via `config['headers']` are merged after the `Authorization` header is set, so they can override default headers if needed — but they cannot override `Authorization`.
</Note>
